...
func MustLoadConfig() *Config {
	...
	flag.StringVar(&config.DatabaseDSN, "d", "user=postgres password=postgres host=localhost port=5432 database=postgres sslmode=disable", "DSN строка для соединения с базой данных") //user=postgres password=postgres host=localhost port=5432 database=postgres sslmode=disable
	flag.Parse()
